...
func main() {
	...
	for _, v := range xtemps {
		templateToFile(v, []string{"Ocean sand is dumb and so are birds", "My brother the mouse cursor", "Papa was a rolling stone and I need surgery"})
	}
}

func templateToFile(templateName string, data interface{}) {
	...
	err = tpl.ExecuteTemplate(f, templateName+".gohtml", data)
	if err != nil {
		log.Fatal("Problem creating html file from template", err)
	}
}
